...
2. Checks whether `ctrld` is running and restarts it if dead. If it will not start at all — a corrupt binary, a config a new release cannot parse — the cycle continues into the fallback chain below rather than ending there, so step 7 is reached
