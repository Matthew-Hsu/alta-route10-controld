...
ID keeps resolving for anyone who has it. The command checks the new ID
first, and if ControlD does not answer for it, refuses the change and exits
non-zero with nothing touched. [How to change
...
Trying a protocol is safe. The command checks the new one first, and if it
does not answer, as DoQ and DoT will not on a network that blocks port 853, it
refuses the switch and exits non-zero while your DNS keeps working.
...
| **Uninstall with Use DoH, `--to on\|off`, the benchmark margin, and the firewall rules' `ctrld` check** | `uninstall.sh` follows Alta's Use DoH, `--force-dns` and `--auto-update` take `--to on` or `--to off`, a benchmark keeps the running protocol unless another is at least 5 ms and 20% faster, and the firewall adds the redirects at boot only while `ctrld` is listening. Each is tested against the real scripts in a sandbox; none has run on a router yet. |
