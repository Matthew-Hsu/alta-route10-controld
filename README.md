...
Re-running the installer later over an existing install is always safe. It
preserves your forced-DNS choice and resolver, and only re-downloads what's
missing.
...
1. Checks whether `ctrld` is running and restarts it if dead
...
4. If DNS fails, **waits for a second consecutive failure** before acting (debounce, to avoid restarting ctrld or churning the protocol on a single transient blip)
