...
- Benchmarking (`bench_protocol`, `bench_stop`, `bench_worth_switching`), shared by all three entry points and never run against production DNS, and the check that a resolver answers over a protocol before anything is changed (`probe_resolver`), which uses the same throwaway `ctrld` on the benchmark port
...
What counts as off is whatever the shell reads as `0` when it sources the file, so `AUTO_UPDATE="0"`, `AUTO_UPDATE='0'`, an indented line, an `export` in front and a trailing comment are all read as off while they are there. Surviving a rewrite is a narrower question: only `AUTO_UPDATE=0` and `AUTO_UPDATE="0"` come back, with or without a trailing comment. A single-quoted value, an indented line and an `export` in front are dropped, and the opt-out goes with them. Write it the way `reconfigure.sh --auto-update` does, or let that command write it.
...
A rewrite keeps a value, never a comment. Comments are not carried in any position, including on their own line, which was always true; what changed is that a comment after a value no longer takes the value with it. `LAN_IFACES_EXCLUDE="br-lan_40"   # guest wifi`, the shape both `README.md` and `docs/troubleshooting.md` tell people to write, used to fail the filter whole and be dropped, which stopped a guest VLAN being excluded within five minutes of an unrelated protocol change. A quoted value is measured to its closing quote rather than having a comment stripped off the end, because a `#` inside quotes belongs to the value; outside quotes a blank has to precede it, since only a `#` that begins a word is a comment to the shell. What the filter writes back is therefore a subset of what it read, which is the direction a filter guarding a sourced file should err in.

Anything the filter refuses is named on stderr as it goes. Every rejected line is something someone typed on purpose, and the cost of dropping one in silence is the guest VLAN above.
...
That filter accepts a narrower set of values than the shell does, so an opt-out written by hand in an unusual shape, single-quoted, indented, or behind an `export`, is dropped at the next rewrite and the weekly update resumes. The rewrite names what it dropped as it goes, so this is visible rather than silent. Three forms are carried: the `AUTO_UPDATE=0` the toggle writes, the quoted `AUTO_UPDATE="0"` a hand-edit produces, and either of those with a comment after it, which is kept for its value and written back without the comment.

Losing an opt-out that way is recoverable and the readouts show it. That is the trade, and it was made deliberately. The alternative was for `write_env_file` to parse the line itself, which it cannot do by sourcing, since it runs on a file that has not been filtered yet and sourcing a value carrying a command substitution would execute it. Matching the line textually instead meant re-deriving the shell's quoting rules, and that code twice concluded the update was off from a file every other reader called on, which silently stops a router patching the binary answering its DNS. Inventing an opt-out is worse than losing one, so the parser was removed and the four shapes it rescued went with it. One has since come back by a different route: a trailing comment is carried because the filter measures where the value ends and writes it back without the comment, which is a question about shape and not about meaning, so nothing has to decide what the line says. The other three are still dropped.
...
Tests every protocol that can be a primary (DoQ, DoH3, DoH and DoT) with real DNS lookups on a separate port (5360) so production DNS is not disrupted. Outputs a formatted table and recommends the fastest protocol, unless the one already running is within 5 ms or 20% of it. A smaller lead is not worth a `ctrld` restart, so the recommendation is then to keep the current one, and `reconfigure.sh --benchmark` keeps it without asking.
